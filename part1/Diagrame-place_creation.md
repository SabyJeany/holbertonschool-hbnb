...
participant DB as
Database Client->>API: POST /places (token, name, description, price, latitude, longitude, amenities)
  API->>API: Extract user_id from token
   ...
   Place->>Place: Validate latitude, longitude loop For each amenity 
   Place->>Place: add_amenity(amenity) 
   Place->>Amenity: Create relationship end 
